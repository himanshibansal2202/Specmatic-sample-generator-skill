...
- [ ] For OpenAPI Backend and BFF samples, endpoint discovery uses a
  ...
  the manifest records the unsupported discovery case and coverage governance is
  omitted.
...
```json
{
  ...
  "endpointDiscovery": {
    "supported": true,
    "mode": "<runtime-accepted-discovery-mode|null>",
    "urlTemplate": "<discovery-url-template|null>",
    "verified": true
  },
...
- `reports`: records the Specmatic report formats and location. For applicable
  ...
  `enforce: true` values.
- `endpointDiscovery`: records whether OpenAPI discovery was supported for the
  selected runtime/framework, the runtime-accepted discovery mode, URL template,
  and whether Specmatic output confirmed discovered runtime endpoints. Use
  `supported: false` and `verified: false` only when runtime/framework support
  is unavailable and the unsupported case is recorded in `learnings`.
