...
| ID | name | type | inventory |
|----|------|------|-----------|
| 10 | "XYZ Phone" | "gadget" | 10 |
| 20 | "Gemini" | "other" | 10 |
...
3. The exact field values for ID 10 should match the spec examples (Specmatic validates schema not values, but matching avoids confusion)
4. New items created via POST should use IDs that don't conflict (start from 100+)
